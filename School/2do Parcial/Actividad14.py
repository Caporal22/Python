...
class Directed_Graph: #Pepe Cantoral
    ...
    #Recorrido bfs
    def bfs(self, start_vertex): #Betta Tech
        visited = set()
        queue = deque()
        result = []
        ...
        return [v.get_name() for v in result]

    #Recorrido dfs
    def dfs(self, start_vertex): #Betta tech
        visited = set()
        result = []
...
#Mandamos como parámetro el Vertex A
print("Recorrido BFS desde A:", g.bfs(vA))
print("Recorrido DFS desde A:", g.dfs(vA))

"""
    Explicación:
    - BFS va por niveles, visitando los vecinos cercanos
    - DFS va hasta el último nivel para ver la profundidad y empieza a retroceder
    - Poniendolo en práctica BFS es mejor porque va en niveles y es más rápido por el número de edges(aristas) que recorre, 
     ya que son menos.  
"""
